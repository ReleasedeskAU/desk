...
export type AlertLifecycleStatusConfig = {
  ...
  isIntake: boolean;
  /** When true, matching live rows stop repeat auto-creates (AV-14). */
...
export type AlertLifecycleTransitionConfig = {
  ...
  isSystem: boolean;
  sortOrder: number;
  /** Optional Checks-tab attachments (main catalog). Feature graph ships ungated. */
  gates?: import("@/lib/alert-lifecycle-gates").AlertLifecycleGateAttachment[];
};
...
function status(
  partial: Omit<AlertLifecycleStatusConfig, "isIntake" | "suppressesRepeatAlerts">
  ...
  const mapped: AlertLifecycleConfig = {
    statuses: candidate.statuses.map((s) => {
      ...
      return {
        ...
        suppressesRepeatAlerts: coerceBoolean(
          s.suppressesRepeatAlerts,
          def?.suppressesRepeatAlerts ?? false
        ),
      };
    }),
