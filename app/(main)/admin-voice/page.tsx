...
/**
 * Voice admin dashboard — usage, per-user minute caps, bans.
 */
function AdminVoicePageInner() {
    ...
    try {
      ...
      for (const u of json.users ?? []) {
        drafts[u.clerkUserId] =
          u.dailyMinutesLimit == null ? "" : String(u.dailyMinutesLimit);
      }
  ...
  const patchUser = async (
    clerkUserId: string,
    body: { banned?: boolean; dailyMinutesLimit?: number | null }
  ) => {
    ...
    <div className="w-full pb-24 font-sans" data-testid="admin-voice-page">
      <div className="mb-8 mt-2">
        ...
        <p className="text-[15px] font-medium leading-relaxed text-gray-500 dark:text-gray-300">
          Track Release Desk Voice usage, set per-user daily minute limits, and
          ban accounts. Changes apply on the next session mint (and mid-session
          via heartbeat).
        </p>
      </div>
      ...
      <div className="mb-6 rounded-xl border border-gray-200 bg-white p-4 text-[13px] dark:border-[var(--border)] dark:bg-[var(--card)]">
        ...
        <ul className="mt-2 space-y-1 text-gray-600 dark:text-gray-300">
          ...
          <li>
            Per-user daily minutes: set below (empty = no personal minutes cap)
          </li>
        </ul>
      </div>
        ...
        <div className="mt-3 flex flex-wrap items-end gap-2">
          ...
          <label className="text-[12px] text-gray-600 dark:text-gray-300">
            ...
            <input
              ...
              className="mt-1 block w-28 rounded-md border border-gray-200 px-2 py-1.5 dark:border-[var(--border)] dark:bg-[var(--card)] dark:text-white"
              placeholder="∞"
            />
          </label>
          <button
        ...
        <table className="w-full text-left text-[13px]">
          <thead className="bg-gray-50 text-gray-500 dark:bg-white/[0.04] dark:text-gray-400">
            <tr>
              ...
              <th className="px-4 py-2.5 font-semibold">Minutes limit</th>
          ...
          <tbody>
            {(data?.users ?? []).length === 0 ? (
              <tr>
                ...
                >
                  No voice usage or policies yet today. Limits apply after a user
                  opens Voice once (or when you set a policy for their Clerk id).
                </td>
              </tr>
            ) : (
              (data?.users ?? []).map((u) => {
                ...
                return (
                  <tr
                    key={u.clerkUserId}
                    className="border-t border-gray-100 dark:border-[var(--border)]"
                  >
                    ...
                    <td className="px-4 py-3 align-top">
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="w-20 rounded-md border border-gray-200 bg-white px-2 py-1.5 text-[13px] dark:border-[var(--border)] dark:bg-[var(--card)] dark:text-white"
                        />
                        <button
                          type="button"
                          disabled={busy}
                          onClick={() => {
                            ...
                            void patchUser(u.clerkUserId, { dailyMinutesLimit });
                          }}
