...
import { alertSourceOptions } from "@/lib/alert-sources";
...
export default function MonitoringAlertDetailPage({
  ...
  const alertConfig =
    (lifecycle.config as AlertLifecycleConfig | null) ??
    createDefaultAlertLifecycleConfig();

  const statusOptions = useMemo(() => {
    const current = row?.status ?? "";
    const next = legalNextAlertStatuses(alertConfig, current);
  ...
  const showTerminalStatusNotice = useMemo(() => {
    ...
    return shouldShowTerminalLifecycleEditNotice({
      ...
      isTerminal: resolveAlertLifecycleStatusRef(alertConfig, current)?.terminal,
    });
  }, [alertConfig, row?.status]);

  const sourceOptions = useMemo(
    () => alertSourceOptions(row?.alertSource),
    [row?.alertSource]
  );
  ...
  const save = async () => {
    ...
    const patchBody = {
      ...
      notes: d.notes || null,
      alertSource: d.alertSource || undefined,
    };
  ...
  const statusRef = resolveAlertLifecycleStatusRef(alertConfig, v.status);
  const openish = statusRef ? !statusRef.terminal : true;
  const unacknowledged = Boolean(statusRef?.isIntake);
  ...
  const workflow = alertWorkflow(v.status, alertConfig);
  ...
  const scope: DetailFact[] = [
    ...
    { label: "Source", value: v.alertSource || "—" },
    { label: "Origin", value: row.autoGenerated ? "Auto" : "Manual" },
  ];
        ...
        d ? (
          <EditableFieldGrid cols={2}>
            ...
            <EditableField
              ...
              onChange={(n) => edit.setField("alertSource", n)}
            />
            <EditableField
              label="Application"
            ...
            <EditableField
              ...
              placeholder="Dismissal justification or working notes…"
      ...
      <DetailDecisionHeader
        status={{
          ...
          caption: statusRef?.terminal
            ? "Alert cleared"
            : `${statusRef?.isIntake ? "Firing on" : "On"} ${v.environmentName || "this environment"}${assignee ? ` with ${assignee}` : ", unowned"}`,
        }}
        signals={signals}
