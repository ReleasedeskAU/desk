...
import { findStafflessConnector, listStafflessIndexErrors, StafflessIdError } from "@/lib/staffless/api";
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const page = await listStafflessIndexErrors(row);
    return NextResponse.json(page);
  } catch (err) {
    if (err instanceof StafflessIdError) {
      return NextResponse.json({ error: err.message }, { status: err.status });
    }
    logger.error("api/connectors/errors", { kind: err instanceof Error ? err.name : "unknown" });
