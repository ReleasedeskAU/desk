...
/**
 * Checks tab for Dependency Lifecycle — attach catalog gates to each move.
 */
import { useEffect, useMemo, useState } from "react";
import { ChevronDown } from "lucide-react";
import {
  DEPENDENCY_LIFECYCLE_GATE_CATALOG,
  DEPENDENCY_LIFECYCLE_GATE_TYPES,
...
export type DependencyGatesPanelProps = {
...
function partition(attachedTypes: Set<string>): {
  attached: DependencyLifecycleGateType[];
  available: DependencyLifecycleGateType[];
} {
  const attached: DependencyLifecycleGateType[] = [];
  const available: DependencyLifecycleGateType[] = [];
  for (const gateType of DEPENDENCY_LIFECYCLE_GATE_TYPES) {
...
function GateGroup(props: {
  title: string;
  empty: string;
  types: DependencyLifecycleGateType[];
  attached: boolean;
  editing: boolean;
  fromKey: string;
  toKey: string;
  onToggleGate: DependencyGatesPanelProps["onToggleGate"];
}) {
  return (
