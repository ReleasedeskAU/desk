...
/**
 * Checks tab for Conflict Lifecycle — attach catalog gates to each move.
 */
import { useEffect, useMemo, useState } from "react";
import { ChevronDown } from "lucide-react";
import {
  CONFLICT_LIFECYCLE_GATE_CATALOG,
  CONFLICT_LIFECYCLE_GATE_TYPES,
...
export type ConflictGatesPanelProps = {
...
function partition(attachedTypes: Set<string>): {
  attached: ConflictLifecycleGateType[];
  available: ConflictLifecycleGateType[];
} {
  const attached: ConflictLifecycleGateType[] = [];
  const available: ConflictLifecycleGateType[] = [];
  for (const gateType of CONFLICT_LIFECYCLE_GATE_TYPES) {
...
/** List Conflict transitions and allow catalog checks to be attached. */
export function ConflictGatesPanel({
  config,
  editing,
  onToggleGate,
}: ConflictGatesPanelProps) {
...
function GateGroup(props: {
  title: string;
  empty: string;
  types: ConflictLifecycleGateType[];
  attached: boolean;
  editing: boolean;
  fromKey: string;
  toKey: string;
  onToggleGate: ConflictGatesPanelProps["onToggleGate"];
}) {
  return (
      ...
      {props.types.length === 0 ? (
        ...
        <ul className="divide-y divide-slate-100 rounded-xl border border-slate-200 dark:divide-white/10 dark:border-[var(--border)]">
          {props.types.map((gateType) => {
            const definition = CONFLICT_LIFECYCLE_GATE_CATALOG[gateType];
            return (
              <li
                key={gateType}
