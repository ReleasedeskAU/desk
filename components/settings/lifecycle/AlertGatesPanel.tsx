...
/**
 * Checks tab for Alert Lifecycle — attach catalog gates to each move.
 */
import { useEffect, useMemo, useState } from "react";
import { ChevronDown } from "lucide-react";
import {
  ALERT_LIFECYCLE_GATE_CATALOG,
  ALERT_LIFECYCLE_GATE_TYPES,
...
export type AlertGatesPanelProps = {
...
function partition(attachedTypes: Set<string>): {
  attached: AlertLifecycleGateType[];
  available: AlertLifecycleGateType[];
} {
  const attached: AlertLifecycleGateType[] = [];
  const available: AlertLifecycleGateType[] = [];
  for (const gateType of ALERT_LIFECYCLE_GATE_TYPES) {
...
/** List Alert transitions and allow catalog checks to be attached. */
export function AlertGatesPanel({
  config,
  editing,
  onToggleGate,
}: AlertGatesPanelProps) {
...
function GateGroup(props: {
  title: string;
  empty: string;
  types: AlertLifecycleGateType[];
  attached: boolean;
  editing: boolean;
  fromKey: string;
  toKey: string;
  onToggleGate: AlertGatesPanelProps["onToggleGate"];
}) {
  return (
      ...
      {props.types.length === 0 ? (
        ...
        <ul className="divide-y divide-slate-100 rounded-xl border border-slate-200 dark:divide-white/10 dark:border-[var(--border)]">
          {props.types.map((gateType) => {
            const definition = ALERT_LIFECYCLE_GATE_CATALOG[gateType];
            return (
              <li
                key={gateType}
