...
/**
 * Checks tab for Risk Lifecycle — attach catalog gates to each move.
 */
import { useEffect, useMemo, useState } from "react";
import { ChevronDown } from "lucide-react";
import {
  RISK_LIFECYCLE_GATE_CATALOG,
  RISK_LIFECYCLE_GATE_TYPES,
...
export type RiskGatesPanelProps = {
...
function partition(attachedTypes: Set<string>): {
  attached: RiskLifecycleGateType[];
  available: RiskLifecycleGateType[];
} {
  const attached: RiskLifecycleGateType[] = [];
  const available: RiskLifecycleGateType[] = [];
  for (const gateType of RISK_LIFECYCLE_GATE_TYPES) {
...
/** List Risk transitions and allow catalog checks to be attached. */
...
function GateGroup(props: {
  title: string;
  empty: string;
  types: RiskLifecycleGateType[];
  attached: boolean;
  editing: boolean;
  fromKey: string;
  toKey: string;
  onToggleGate: RiskGatesPanelProps["onToggleGate"];
}) {
  return (
      ...
      {props.types.length === 0 ? (
        ...
        <ul className="divide-y divide-slate-100 rounded-xl border border-slate-200 dark:divide-white/10 dark:border-[var(--border)]">
          {props.types.map((gateType) => {
            const definition = RISK_LIFECYCLE_GATE_CATALOG[gateType];
            return (
              <li
                key={gateType}
