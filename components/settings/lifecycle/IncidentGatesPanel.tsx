...
/**
 * Checks tab for Incident Lifecycle — attach catalog gates to each move.
 */
import { useEffect, useMemo, useState } from "react";
import { ChevronDown } from "lucide-react";
import {
  INCIDENT_LIFECYCLE_GATE_CATALOG,
  INCIDENT_LIFECYCLE_GATE_TYPES,
...
export type IncidentGatesPanelProps = {
...
function partition(attachedTypes: Set<string>): {
  attached: IncidentLifecycleGateType[];
  available: IncidentLifecycleGateType[];
} {
  const attached: IncidentLifecycleGateType[] = [];
  const available: IncidentLifecycleGateType[] = [];
  for (const gateType of INCIDENT_LIFECYCLE_GATE_TYPES) {
...
/**
 * List Active/Inactive incident transitions; expand a row to attach checks.
 */
export function IncidentGatesPanel({
  config,
  editing,
  onToggleGate,
}: IncidentGatesPanelProps) {
...
function GateGroup(props: {
  title: string;
  empty: string;
  types: IncidentLifecycleGateType[];
  attached: boolean;
  editing: boolean;
  fromKey: string;
  toKey: string;
  onToggleGate: IncidentGatesPanelProps["onToggleGate"];
}) {
  return (
