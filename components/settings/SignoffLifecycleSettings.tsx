...
/**
 * Lifecycle → Sign-offs — configure statuses, transitions, and type mandatory flags.
 */
...
function cloneConfig(config: SignoffLifecycleConfig): SignoffLifecycleConfig {
  return {
...
/**
 * Sign-off lifecycle settings panel (statuses + transitions + types).
 */
export function SignoffLifecycleSettings() {
  const [baseline, setBaseline] = useState(createDefaultSignoffLifecycleConfig);
  const [draft, setDraft] = useState(createDefaultSignoffLifecycleConfig);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/signoff-lifecycle-config", {
        credentials: "same-origin",
      });
      if (!res.ok) throw new Error("Failed to load");
      const data = (await res.json()) as { config: SignoffLifecycleConfig };
      const config = data.config ?? createDefaultSignoffLifecycleConfig();
      setBaseline(cloneConfig(config));
      setDraft(cloneConfig(config));
    } catch {
      setError("Could not load sign-off lifecycle configuration.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <div className="space-y-5" data-testid="signoff-lifecycle-settings">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex min-w-0 items-start gap-3">
          <span className="mt-0.5 flex h-10 w-10 items-center justify-center rounded-xl bg-violet-500/12 text-violet-700 dark:text-violet-300">
      ...
      <div
        ...
        role="note"
      >
        <p className="font-semibold">Quick help · Sign-offs</p>
        <ul className="mt-1.5 list-disc space-y-1 pl-4">
          <li>
            Pending can move to Approved, Rejected, Approved with Conditions, or Withdrawn
            (Flexible).
          </li>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <LifecycleToggle
                      ...
                      onCheckedChange={(required) => {
                        const enforcement: SignoffLifecycleEnforcement = required
                          ? "required"
                          : "flexible";
                        setDraft((prev) => ({
                          ...prev,
          ...
          {sortedTypes.map((type) => (
            ...
            >
              <div className="min-w-0">
                ...
                <p className="mt-1 font-mono text-[11px] text-slate-400">
                  {type.key}
                  {type.releaseField ? ` · field: ${type.releaseField}` : " · no field"}
                </p>
              </div>
              <div className="flex flex-wrap items-center gap-3">
                <LifecycleToggle
