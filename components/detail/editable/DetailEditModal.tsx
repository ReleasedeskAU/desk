    ...
    >
      ...
      >
        <div className="flex shrink-0 items-start justify-between gap-3 border-b border-slate-100 px-5 py-4 dark:border-[var(--border)]">
          <div>
            <h2 id="detail-edit-title" className="text-lg font-semibold text-gray-900 dark:text-white">
              {title}
            </h2>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
          ...
          {error ? <p className="mt-3 text-sm text-error-600 dark:text-rose-400">{error}</p> : null}
        </div>

        <div className="flex shrink-0 justify-end gap-2 border-t border-slate-100 px-5 py-4 dark:border-[var(--border)]">
          <button type="button" className={taBtnSecondary} onClick={onCancel} disabled={saving}>
            Cancel
          </button>
          <button
            type="button"
