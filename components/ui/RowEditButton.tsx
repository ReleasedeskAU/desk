 ...
 */
export function RowEditButton({ recordLabel, disabled, href, onClick }: RowEditButtonProps) {
  const label = `Edit ${recordLabel}`;
  if (href) {
    return (
      <ProgressLink href={href} aria-label={label} className={cn(editButtonClass, disabled && "pointer-events-none opacity-40")}>
