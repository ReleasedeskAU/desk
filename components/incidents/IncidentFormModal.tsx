...
export function IncidentFormModal({
  ...
  useEffect(() => {
    ...
    return () => ac.abort();
  }, [open, defaultStatus, lockRelatedReleaseCode, preferredApplicationId]);
  ...
  if (created) {
    return (
      ...
      >
        <SummaryRow label="Incident ID" value={created.incidentCode} mono />
        ...
        <SummaryRow label="Impact" value={created.impact} />
      </CreateConfirmation>
    );
  }

  const lockedRelease = releases.find((item) => item.releaseCode === form.relatedReleaseCode);
  const scoped = Boolean(lockRelatedReleaseCode);
  ...
  return (
    <CreateModalShell
      ...
      description={
        scoped
          ? "This incident is tied to the open release. Incident ID is assigned automatically."
          : "Fields marked * are required. Incident ID is generated by the server."
      }
      onClose={onClose}
      footer={
        <>
    ...
    >
      ...
      <form id="incident-create-form" onSubmit={submit} className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2">
        {scoped ? (
