...
type Props = {
  ...
  /** Enabled labels from drift lifecycle config. */
  statusOptions?: string[];
  /** Enabled default status from drift lifecycle config. */
  defaultStatus?: string;
};

/** Creates a validated drift with real org/application/release/environment dependencies. */
export function DriftFormModal({
  open,
  onClose,
  onCreated,
  categoryOptions,
  ...
  const loc = (control: string) => locatorToken("drift_create", control);
    ...
    return (
      <ModalFrame onClose={onClose} labelledBy="drift-created-title">
        ...
        <div className="mt-5 flex flex-wrap justify-end gap-2">
          <button type="button" className={taBtnSecondary} onClick={() => { setCreated(null); setForm(emptyForm(defaultStatus || "Open")); }} {...controlLoc("drift_create_another")}>
            Create another
          </button>
          <ProgressLink href={`/drifts/${created.id}`} className={cn(taBtnSecondary, "inline-flex items-center")} {...controlLoc("drift_created_view")}>
            View Drift
          </ProgressLink>
          <button type="button" className={taBtnPrimary} onClick={onClose} {...controlLoc("drift_created_close")}>Close</button>
        </div>
      </ModalFrame>
    );
  }
  ...
  return (
    <ModalFrame onClose={onClose} labelledBy="new-drift-title" wide>
      ...
      <form onSubmit={submit} className="mt-4 grid gap-3 sm:grid-cols-2">
        <SelectField id={loc("department")} label="Department" required value={form.departmentId} error={fieldErrors.departmentId}
        ...
        <SelectField id={loc("application")} label="Application" required value={form.applicationId} error={fieldErrors.applicationId}
        ...
        <SelectField id={loc("release")} label="Release" required value={form.releaseId} error={fieldErrors.releaseId}
        ...
        <SelectField id={loc("environment")} label="Environment" required value={form.environmentName} error={fieldErrors.environmentName}
        ...
        <SelectField id={loc("drift_type")} label="Drift type" required value={form.driftType} error={fieldErrors.driftType}
        ...
        <TextField id={loc("drift_category")} label="Drift category" value={form.driftCategory} onChange={(event) => set("driftCategory", event.target.value)}
        ...
        <SelectField id={loc("severity")} label="Severity" required value={form.severity} error={fieldErrors.severity}
        ...
        <SelectField id={loc("status")} label="Status" required value={form.status} error={fieldErrors.status}
          onChange={(event) => set("status", event.target.value)}>
          {statusOptions.map((value) => <option key={value} value={value}>{value}</option>)}
        </SelectField>
        <TextField id={loc("detected_date")} label="Detected date" type="date" required value={form.detectedDate} error={fieldErrors.detectedDate}
          onChange={(event) => set("detectedDate", event.target.value)} />
        <TextField id={loc("eta_to_fix")} label="ETA to fix" type="date" value={form.etaToFix}
          onChange={(event) => set("etaToFix", event.target.value)} />
        <TextareaField id={loc("description")} label="Description" required value={form.description} error={fieldErrors.description}
          onChange={(event) => set("description", event.target.value)} />
        <TextareaField id={loc("impact_on_release")} label="Impact on release" value={form.impactOnRelease}
          onChange={(event) => set("impactOnRelease", event.target.value)} />
        <TextareaField id={loc("remediation_action")} label="Remediation action" value={form.remediationAction}
...
function FormError({
  ...
  return (
    <FormAlertDialog
      alert={buildFormSaveAlert(null, message, { entityLabel: "drift" })}
      onDismiss={onDismiss}
    />
  );
}
