...
import { VoiceGuideOverlay } from "@/components/voice/VoiceGuideOverlay";
import { registerVoiceGuideSidebarExpand } from "@/lib/voice/guide-ui";
...
function ShellInner({ children }: { children: React.ReactNode }) {
  ...
  // Guide needs the rail wide so tab targets are visible; overlay must sit ABOVE sidebar z-index.
  useEffect(() => {
    return registerVoiceGuideSidebarExpand(() => {
      setIsHovered(true);
    });
  }, [setIsHovered]);
  ...
  return (
    <ChatProvider>
      <div className="relative min-h-screen materio-page-bg">
        <Sidebar />
        <Backdrop />
        {/* Above sidebar (z-100) so pointer/highlight are actually visible on nav items. */}
        <VoiceGuideOverlay />
        <div
          className={cn(
